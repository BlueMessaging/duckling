(
  ...
  ; same thing, with "de" in between like "domingo de la semana pasada"
  "intersect by `de` latent"
  [(dim :time #(not (:latent %))) #"(?i)de" (dim :time #(:latent %))] ; sequence of two tokens with a time fn
  (intersect %1 %3)
...
; Holiday TODO: check online holidays
  ...
  "del <year> latent"  ; latin america mostly
  [#"(?i)del( a[ñn]o)?" (integer -10000 999)]
  (year (:value %2))
